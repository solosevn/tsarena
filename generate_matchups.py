...
# Default refusal text â shown to voters when a model refuses to respond
...
# Maps TSArena display name â OpenRouter model identifier
...
MODEL_ID_MAP = {
    ...
    # ââ NOT on OpenRouter (skipped) ââ
    "Yi 1.5 34B":               None,
    "Trinity Large":            None,
}
...
async def supabase_rpc(client: httpx.AsyncClient, function_name: str, params: dict) -> dict:
    ...
    if resp.status_code >= 400:
        print(f"  ERROR: RPC {function_name} â {resp.status_code}: {resp.text[:200]}")
        return None
    return resp.json()
...
async def supabase_query(client: httpx.AsyncClient, table: str, params: dict = None):
    ...
    if resp.status_code >= 400:
        print(f"  ERROR: GET {table} â {resp.status_code}: {resp.text[:200]}")
        return None
    return resp.json()
...
async def call_openrouter(client: httpx.AsyncClient, model_id: str, prompt_text: str) -> str:
    ...
    try:
        ...
        if response_text:
            ...
            # Model returned empty/null content â treat as refusal
        ...
        print(f"    EXCEPTION: {model_id} â {e}")
...
def create_model_pairs(models: list, count: int, zero_battle_priority: bool = False) -> list:
    ...
    for _ in range(count):
        ...
        # Random A/B assignment â prevents position bias
...
async def generate_battles(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if len(models) < 2:
            print("ERRORR: Need at least 2 mapped models to generate battles.")
            sys.exit(1)
        ...
        for i, ((model_a, model_b), prompt) in enumerate(zip(pairs, selected_prompts)):
            ...
            print(f"    Model A: {model_a['name']} â {or_id_a}")
            print(f"    Model B: {model_b['name']} â {or_id_b}")

            if dry_run:
                print(f"    â SKIP (dry run)\n")
                successes += 1
                continue

            # Call Model A
            print(f"    â Calling {model_a['name']}...", end=" ", flush=True)
            ...
            # Call Model B
            print(f"    â Calling {model_b['name']}...", end=" ", flush=True)
            ...
            if is_error_a or is_error_b:
                print(f"    â SKIPPED â technical error (not a content refusal)")
                failures += 1
                run_log.append({
                    "battle": battle_num,
                    "status": "error",
            ...
            if result:
                ...
                print(f"    â Battle #{battle_num} inserted ({status})")
                run_log.append({
                    "battle": battle_num,
                    "status": status,
                    "model_a": model_a["name"],
                ...
                failures += 1
                print(f"    â Failed to insert battle #{battle_num}")
                run_log.append({
                    "battle": battle_num,
                    "status": "insert_error",
                    "model_a": model_a["name"],
    ...
    print(f"  Total:      {count}")
    print(f"  Battle range: #{next_battle_num} â #{next_battle_num + successes - 1}")
    print(f"{'='*60}\n")
